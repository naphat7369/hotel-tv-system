...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // ── MDM WebSocket ────────────────────────────────────────────────
        webSocketClient = WebSocketClient(this, Config.getServerUrl(this))
    ...
    private fun showError(detail: String = "") {
        runOnUiThread {
            ...
            loadingSubtitle.text = if (detail.isNotBlank())
                ...
                "Reconnecting to the hotel network…"
        }
        scheduleRetry()
    }
    ...
    inner class AndroidTVBridge {
        ...
        /**
         * Allows React to read the persistent Device ID generated by the Android App.
         */
        @JavascriptInterface
        fun getDeviceId(): String = Config.getDeviceId(this@MainActivity)
            ...
            .also { player ->
                ...
                player.addListener(object : Player.Listener {
                    ...
                    override fun onPlayerError(error: androidx.media3.common.PlaybackException) {
                        Log.e("ExoPlayer", "Playback error (retry $retryCount/$MAX_RETRIES): ${error.message}", error)
                        if (retryCount < MAX_RETRIES) {
                            retryCount++
                            Log.d("ExoPlayer", "Auto-retrying in 3 seconds (attempt $retryCount)...")
                            Handler(Looper.getMainLooper()).postDelayed({
                                try {
                                    ...
                                    Log.e("ExoPlayer", "Retry failed: ${e.message}")
                                }
                            }, 3000)
                        } else {
                            Log.e("ExoPlayer", "Max retries reached. Notifying WebView.")
                            runOnUiThread {
                                webView.evaluateJavascript(
    ...
    private fun reportNetworkStatus() {
        try {
            ...
            val report = JSONObject().apply {
                put("deviceId",   Config.getDeviceId(this@MainActivity))
    ...
    private fun startKioskMode() {
        if (isDeviceOwner() && !isKioskActive) {
            try {
                startLockTask()
                isKioskActive = true
            } catch (e: Exception) {
