...
import { getRequestStats } from './request.routes';
...
router.get('/overview', async (req: Request, res: Response) => {
  try {
    ...
    const allDevices = Array.from(connectedDevices.values());
...
router.get('/reports', async (req: Request, res: Response) => {
  try {
    ...
    const whereClause: any = { hotelId: MOCK_HOTEL_ID };
    ...
    const channelStats: Record<string, { name: string, duration: number }> = {};
    channelWatch.forEach(e => {
      try {
        if (e.value && e.durationSeconds) {
          const val = JSON.parse(e.value);
          if (val.channelId && val.name) {
            if (!channelStats[val.channelId]) channelStats[val.channelId] = { name: val.name, duration: 0 };
    ...
    const appStats: Record<string, { count: number; duration: number }> = {};
    events.forEach(e => {
