    ...
    io.on('connection', (socket) => {
        ...
        socket.on('disconnect', () => {
            ...
            if (currentDeviceId) {
                ...
                const existing = exports.connectedDevices.get(deviceIdToDisconnect);
                if (existing && existing.socketId === socket.id) {
                    const ipToPing = existing.ipAddress;
                    const verifyStatus = async () => {
                        const check = exports.connectedDevices.get(deviceIdToDisconnect);
                        ...
                        if (check && (check.socketId === socket.id || check.socketId === undefined)) {
                            if (ipToPing) {
                                if (await checkDeviceAlive(ipToPing)) {
                                    ...
                                    exports.connectedDevices.set(deviceIdToDisconnect, check);
                                ...
                                const nextTimer = setTimeout(() => {
                                    const finalCheck = exports.connectedDevices.get(deviceIdToDisconnect);
                                    if (finalCheck && finalCheck.socketId === undefined) {
                                        finalCheck.isOnline = false;
                                        finalCheck.lastSeen = new Date().toISOString();
                                        exports.connectedDevices.set(deviceIdToDisconnect, finalCheck);
                                        broadcastDeviceList(io);
