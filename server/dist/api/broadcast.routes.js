...
router.post('/messages', async (req, res) => {
    try {
        ...
        const newMessage = await prisma.savedMessage.create({
            data: {
...
router.delete('/messages/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await prisma.savedMessage.delete({ where: { id } });
...
router.get('/active', async (req, res) => {
    try {
        const broadcasts = await prisma.activeBroadcast.findMany({
            where: { hotelId: MOCK_HOTEL_ID },
            orderBy: { createdAt: 'desc' }
        });
...
router.delete('/active/:id', async (req, res) => {
    try {
        ...
        const broadcast = await prisma.activeBroadcast.findUnique({ where: { id } });
        if (!broadcast) {
            return res.status(404).json({ error: 'Broadcast not found' });
        }
        // Delete it
        await prisma.activeBroadcast.delete({ where: { id } });
...
router.post('/send', async (req, res) => {
    try {
        ...
        const activeBroadcast = await prisma.activeBroadcast.create({
            data: {
                hotelId: MOCK_HOTEL_ID,
                message,
                type: type || 'default',
