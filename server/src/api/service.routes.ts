...
// ── WebSocket helper ────────────────────────────────────────────────────────
const emitMenuRefresh = (req: Request, action: string, item?: any) => {
  req.app.get('io')?.emit('refresh_guest_menu', { action, item });
};

// ── Scheduling helper ───────────────────────────────────────────────────────
// Used by GET endpoint to filter items whose schedule window is active
const isScheduleActive = (item: { activeFrom: Date | null; activeUntil: Date | null }): boolean => {
  const now = new Date();
...
router.get('/menu-items', async (req: Request, res: Response) => {
  try {
    ...
    const where: any = { hotelId: MOCK_HOTEL_ID, isActive: true };
...
router.delete('/menu-items/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (item?.bgImage) {
      ...
      if (fs.existsSync(fullPath) && localPath.startsWith('menu-images/')) {
...
// ──────────────────────────────────────────────────────────────────────────────
router.put('/menu-items/reorder', async (req: Request, res: Response) => {
  try {
    ...
    await Promise.all(
      items.map(({ id, sortOrder }) =>
        prisma.guestMenuItem.update({ where: { id }, data: { sortOrder } })
      )
    );
...
router.get('/room-service', (_req: Request, res: Response) => {
